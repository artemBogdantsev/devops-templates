...
* See [variables.tf](./variables.tf) for all the
variables you can set on this module.
* See [outputs.tf](./outputs.tf) for all the variables
...
The module in the [`vpc-network`](../vpc-network)
is a useful tool for configuring your VPC network and 
subnetworks in GCP.
...
While using a secondary IP range is recommended [in order to to separate cluster master and pod IPs](../vpc-network#how-is-a-secondary-range-connected-to-an-alias-ip-range),
